...
def cust_doc_similarity():
    try:
        ...
        # Ensure both documents have the same number of sentences
        if len(embeddings1) != len(embeddings2):
            return jsonify({'error': 'Documents must have the same number of sentences'}), 400
        
        # Calculate cosine similarity for each sentence pair
        similarities = [
            np.dot(embeddings1[i], embeddings2[i]) / (np.linalg.norm(embeddings1[i]) * np.linalg.norm(embeddings2[i]))
